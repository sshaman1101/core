...
func ResponseActiveDeals(ctx context.Context, cfg *connor.Config, dealsDb []*records.DealDb, m sonm.MarketClient, dealCli sonm.DealManagementClient, tMng sonm.TaskManagementClient, imageMonero string) error {
	for _, dealDb := range dealsDb {
		if dealDb.Status == 1 && dealDb.DeployStatus == 4 {
			...
			ReinvoiceOrder(ctx, cfg, m, &sonm.Price{PerSecond: deal.GetPrice()}, bench, "Reinvoice")
		} else {
		...
		if orderDb.ButterflyEffect != 3 { // 3 :: Cancelled (is not)
			if order.GetOrderStatus() == 2 { // 2 :: Active
				...
				if commandPrice == 1 || commandPrice == -1 {
					...
					ReinvoiceOrder(ctx, cfg, m, &sonm.Price{PerSecond: sonm.NewBigInt(pricePerSecForPack)}, bench, "Reinvoice OldOrder: "+tag)
